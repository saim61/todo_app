defmodule TodoApp.ItemsTest do
  ...
  describe "todos" do
    ...
    test "create_todo/1 with valid data creates a todo" do
      valid_attrs = %{date: ~D[2023-09-16], description: "some description", priority: :"low,medium,high", title: "some title"}

      assert {:ok, %Todo{} = todo} = Items.create_todo(valid_attrs)
      assert todo.date == ~D[2023-09-16]
      assert todo.description == "some description"
      assert todo.priority == :"low,medium,high"
    ...
    test "update_todo/2 with valid data updates the todo" do
      todo = todo_fixture()
      update_attrs = %{date: ~D[2023-09-17], description: "some updated description", priority: :"low,medium,high", title: "some updated title"}

      assert {:ok, %Todo{} = todo} = Items.update_todo(todo, update_attrs)
      assert todo.date == ~D[2023-09-17]
